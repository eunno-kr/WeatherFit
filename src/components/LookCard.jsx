...
function OutfitCard({ outfit, index, theme }) {
  return (
    <article className="border border-black/10 bg-[#FFFDF7] p-5 transition hover:border-black/30">
      <div className="flex items-center justify-between gap-3">
        <div>
          <div className="font-display text-[10px] font-bold" style={{ color: theme.accent }}>
            LOOK {index + 1}
          </div>
      ...
      <div className="mt-4 grid gap-2">
        {SLOTS.map(([label, key]) => (
          <div key={key} className="grid grid-cols-[56px_1fr] gap-3 text-sm">
            <span className="font-display text-[11px] font-bold" style={{ color: theme.accent }}>
              {label}
            </span>
...
export default function LookCard({ look, theme }) {
  return (
    <section className="mt-5 border p-5 sm:p-6" style={{ background: theme.soft, borderColor: `${theme.accent}33` }}>
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="font-display text-[11px] font-bold" style={{ color: theme.accent }}>
            TODAY'S LOOK
          </div>
          <h2 className="mt-1 text-xl font-semibold">오늘 바로 입는 3가지 코디</h2>
        </div>
        <div className="text-xs text-[#6B665C]">일반 추천</div>
      </div>
